	...
	case S3C_IRQTYPE_EDGE:
		if (irq_data->parent_irq ||
			...
			irq_set_chip_and_handler(virq, &s3c_irq_level_chip,
						 handle_edge_irq);
			//printk(KERN_CRIT "s3c2416\n");
		} else {
			irq_set_chip_and_handler(virq, &s3c_irq_chip,
						 handle_edge_irq);
			//printk(KERN_CRIT "normal\n");
		}
		break;
				       ...
				       unsigned long address)
{
	...
	case 0x560000a4:
		...
		irq_num = 20;
		irq_start = S3C2410_IRQ(32); //Start of extra interrupts.
		irq_offset = 4;
		break;
	default:
	...
	pr_err("[GAUN] Adding domain size: %u,first_irq: %u,first_hwirq: %u",
								irq_num,
								irq_start,
								irq_offset);
